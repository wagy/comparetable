	...
	switch($class) {
		...
		case ListView:		// Array with items to be checked
			...
			foreach($values as $index)
				wbtemp_set_listview_item_checked($ctrl, $index, $item);
			break;
...
function wb_sys_dlg_open($parent=null, $title=null, $filter=null, $path=null, $filename=null, $flags = null, $chDir=true)
{
	$filter = _make_file_filter($filter ? $filter : $filename);
	if ($chDir)
	{
		//Achtung wbtemp_sys_dlg_open veränder das aktuelle Verzeichnis
		$wsShell = new COM("WScript.Shell");
		$actPath = $wsShell->CurrentDirectory;
		$ret = wbtemp_sys_dlg_open($parent, $title, $filter, $path, $flags);
		$wsShell->CurrentDirectory = $actPath;
		$wsShell = null;
		return $ret;
	}
